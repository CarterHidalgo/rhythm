...
public class UCI {
    ...
    private static final Pattern positionPattern = Pattern.compile("(?:fen (?<fen>.* \\d+ \\d+)|startpos)(?: moves (?<moves>.*))?");

    private static final Pattern[] goNumeralPatterns = new Pattern[] {
    ...
    private static void go(String params) {
        ...
        GoParameters goParameters = new GoParameters(
            options.get(0),
            options.get(1),
            options.get(2),
            options.get(3),
        ...
        if (goParameters.getPerft().isPresent()) {
            ...
            int nodes = Model.perft(goParameters.getPerft().get());
